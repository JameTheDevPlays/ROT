...
mainApp.post('/', async (req, res) => {
    ...
    if (!cookie) return console.error("ZCLO Auto Spoofer - Invalid cookie and couldn't find in registry");
    ...
    for (const [name, id] of Object.entries(req.body.ids)) {
      ...
      while (i < 5) {
        ...
        console.log(name, id, ' Retrying, hold.');
      }
    }
    ...
    console.log(name, id, ' Failed, sorry.');
    failedIDs.push(id);
  }

    }
    console.log('ZCLO Auto Spoofer - Finished Reuploading Animations | Thanks for choosing us!');
    console.log(failedIDs);
    console.log(remapped);
    workingStill = false;
});

mainApp.listen(6969, () => console.log(`ZCLO Auto Spoofer \n- Version ${CurrentVer}`));